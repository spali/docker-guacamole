...
echo "---------------------------------------------------------------------------------------"
apk --update add libtool autoconf automake git build-base \
	libpng-dev libjpeg-turbo-dev
curl "http://ftp.unicamp.br/pub/apache/maven/maven-3/$MAVEN_VERSION/binaries/apache-maven-$MAVEN_VERSION-bin.tar.gz" | tar -xzC /tmp
mv /tmp/apache-maven-* /tmp/maven
#apt-get update
...
autoreconf -fi
./configure --with-init-dir=/etc/init.d || cat config.log && return false
make
make install
...
curl -o /var/lib/guacamole/classpath/postgresql-${PCJ_VER}.jdbc41.jar https://jdbc.postgresql.org/download/postgresql-${PCJ_VER}.jdbc41.jar
# Download dependancies for mysql authentication module
curl -o /tmp/guacamole-modules/mysql-connector-java-${MCJ_VER}.tar.gz http://dev.mysql.com/get/Downloads/Connector-J/mysql-connector-java-${MCJ_VER}.tar.gz
